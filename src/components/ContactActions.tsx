...
import { contactConfig, getEmailUrl, getWhatsAppUrl } from "@/lib/contact";
import { trackContactEvent, type ContactAnalyticsSource } from "@/lib/analytics";
...
export function ContactActions({
  ...
  description = "Conversa con nuestro asistente de IA por WhatsApp o escríbenos por correo. Te ayudaremos a evaluar TCDX Compliance para tu empresa.",
  ...
  const emailUrl = getEmailUrl(
    "Información sobre TCDX Compliance",
    "Hola, me gustaría recibir más información sobre TCDX Compliance."
  );

  const handleWhatsAppClick = () => {
    trackContactEvent("contact_whatsapp_click", { page_path: pathname ?? "/", source });
  };

  const handleEmailClick = () => {
    trackContactEvent("contact_email_click", { page_path: pathname ?? "/", source });
  };
